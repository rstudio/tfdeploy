---
title: "tfdeploy: Deploys TensorFlow Models"
output:
  github_document:
    fig_width: 9
    fig_height: 5
...
`tfdeploy` provies tools for deploying TensorFlow SavedModels into local and remote
services.
...
This local webapi can be accessed with `curl`, `httr`, `jsonlite`, etc. or
using the `predict_savedmodel()` deployment client.
...
We can make use of the [pixels](https://CRAN.R-project.org/package=pixels) HTMLWidget 
to manually collect a vector of pixels and perform a prediction over the model:
...
Predictions over models deployed on the [Google Cloud Machine Learning Engine](https://cloud.google.com/ml-engine/)
can be easily accessed by running:
